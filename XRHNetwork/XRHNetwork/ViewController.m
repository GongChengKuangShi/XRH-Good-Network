...
- (void)viewDidLoad {
    ...
    _url = [NSString stringWithFormat:@"http://api.tianapi.com/%@/?key=52a9ca67f0f797110011bb98770a3163&num=%d",@"keji",10];
...
- (void)getData:(BOOL)isOn url:(NSString *)url
{
    ...
    if(isOn)
    {
        ...
        [PPNetworkHelper GET:url parameters:para responseCache:^(id responseCache) {
            ...
            self.cacheData.text = [self convertToJSONData:responseCache];
        } success:^(id responseObject) {
            // 2.再请求网络数据
            NSLog(@"responseObject ++++ %@",responseObject);
            self.networkData.text = [self convertToJSONData:responseObject];
        } failure:^(NSError *error) { 
    ...
    [PPNetworkHelper networkStatusWithBlock:^(PPNetworkStatusType networkStatus) {
        ...
        switch (networkStatus) {
            ...
            case PPNetworkStatusReachableViaWiFi:
                [self getData:[[NSUserDefaults standardUserDefaults] boolForKey:@"isOn"] url:_url];
                PPLog(@"有网络,请求网络数据");
                break;
        }
...
- (IBAction)isCache:(UISwitch *)sender {
    ...
    [self getData:sender.isOn url:_url];
}
...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event {
    [self.cacheData endEditing:YES];
    [self.networkData endEditing:YES];
}
